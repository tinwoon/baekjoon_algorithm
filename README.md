...
#### 127. ~의 배수를 찾는 문제는 분명하게 %연산자의 최적화를 의미한다.

- (a+b)%c, (a-b)%c. (a*b)%c등 에서 %를 통해 최적화하라는 것을 의미한다.



#### 128. dp문제는 dp문제라는 확신이 있어도 dp로 어떻게 설계할까가 먼저가 아니라 dp를 쓰지 않는다면 어떻게 풀지를 고민한 후 거기다가 dp를 넣어야한다.

- 백준 19623번의 회의실 배정 4의 경우 dp로 풀라는 점이 자명하게 알 수 있다.
- 하지만 만약 dp로 사용하지 않는다면 어떻게 풀지를 고민하면 쉽게 답을 찾을 수 있다.
- 이 문제를 dp를 사용하지 않는다면  N개의 회의실을 사용할지, 안할지를 고민할 수 있으므로 2^N의 브루트 포스로 풀 수 있다.
- 하지만 해당 경우의 수에서 2^N-1의 최대값을 안다면 그 값을 기반으로 2^N의 값을 풀 수 있다는걸 알 수 있다.
- 더 나아가 브루트 포스 자체도 2^N이 되니 다른 방법을 생각해보자
- 더 생각해보니 sorting을 끝난 시간순으로 정렬 한 후 구하면 풀 수 있음을 알 수 있다.
...
- 다음 블로그에 더 자세하게 내용이 정리되어 있다.(https://www.acmicpc.net/blog/view/9)
...
#### 136. 배열 선언 자체가 대입 연산자 만큼의 cost가 사용된다는 점도 복잡도에 영향을 줘야한다.
...
#### 140. Tree의 root는 정해져있지 않다.

- 모든 Tree들의 각각의 노드들은 현재 Root가 아니더라도 Root가 될 수 있다는 점을 기억할 것
